...
- **Per-run font info** — each `FRichTextRun` carries its own `FSlateFontInfo` (family, size, and typeface); `OnPaint` iterates runs, swapping `TypefaceFontName` for bold/italic variants; underline and strikethrough drawn as line primitives; a toolbar dropdown and size spinner let users change font family and size with immediate apply-to-selection support
